...
from algorithms.decision_tree_classifier.cl_node import Node
from algorithms.decision_tree_classifier.cl_leaf import Leaf



class DecisionTreeClassifier:
    
    def __init__(self, criterion="entropy", max_depth=3):
        assert criterion == "entropy", f"only criterion 'entropy' is implemented."
        ...
        self.f_criterion = self.entropyCriterion
        ...
        while not q_incomplete_nodes.empty():
            ...
            if current_depth >= self.max_depth:
                ...
                if not incomplete_child_node_a.y.empty:
                    label_a = incomplete_child_node_a.y.value_counts().idxmax()[0]
                label_b = 0 
                if not incomplete_child_node_b.y.empty:
                    label_b = incomplete_child_node_b.y.value_counts().idxmax()[0]
